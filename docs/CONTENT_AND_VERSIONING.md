...
- 根据 `C:\Users\AA\Desktop\截图\05 Building a Layout` 的 28 张截图，在字段页签中新增 Layouts：布局列表、新建与顺序参考、启用 Copy From Previous、三种保存方式和删除。
- 新增布局编辑器：添加或删除分区，添加截图示例字段，设置只读、显示方式、字段名、自定义标签、重复列数和允许复制上一份；支持上下文说明编辑、保存、Save and Close、关闭时保存/放弃/继续编辑。
- 新增布局预览：使用演示文档填写布局，Issues 使用现有多选项，Custodian 只读；启用复制上一份后可复制允许复制的字段值。
- Layouts 与 Fields 共用当前会话状态；字段删除前检查布局引用，避免留下悬挂字段；载入截图示例字段不会覆盖同名字段和已有选项。
- 实现边界：拖放网格、真实富文本编辑器、真实租户权限和后端审计未模拟；编辑器使用本地按钮和安全 HTML 清理演示对应流程。类型检查、构建、Layout 浏览器流程、Field 回归及 390px 窄屏检查通过。
- 修改前 v0.5 文件已移入 `工作准备\历史版本\04_交互原型\Layout更新前_v0.5_2026-09-23`；验证脚本、截图接触页及迁移记录保留在 `工作准备\草稿与中间产物\LayoutField_2026-09-23`。本轮尚未提交 Git。
